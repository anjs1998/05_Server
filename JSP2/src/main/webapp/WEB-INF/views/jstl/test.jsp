...
<body>
	...
	<h3>2. 변수 제거하기(==속성 제거 ) - c: remove 태그</h3>
	...
	<pre>
		[c:choose 태그]
		- 내부에 조건문 태그를 작성하겟다는 태그
		-> 내부에 c:when, c:otherwise 태그만 작성 가능
		
		*주의 사항*
		바로 한단계 아래에는
		c:when, c:otherwise, 공백, jsp용 주석만 작성할수있고
		나머지 모두 에러 발생!!
		<!-- html용 주석 :choose 태그 안에 html용 주석을 쓰면 에러가 뜬다. -->
	...
	<c:choose>
		...
		<%-- empty 연산자
			...
			1) null인 경우는 true
			2) 연산대상이 배열/컬렉션ㅇ니데 내부에 아무 요소도 없는경우 true
   ...
   6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
	...
	<ul>
		...
		<li>
		<%--접두사 fn이 추가된 El구문 사용
		fn(function,기능) : 문자열, 컬렉션 관련 기능 제공하는 JSTL_EL
		...
		<c:forEach var="name" items="${nameList}" varStatus="vs">
			...
			<li>
				<ul>
					...
					<li>현재 요소 : ${nameList[vs.current]}</li>
				</ul>
			</li>
