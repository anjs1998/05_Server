...
public class TodoListDAOImpl implements TodoListDAO {
	...
	public TodoListDAOImpl() {
		...
		try {
			String filePath = TodoListDAOImpl.class.getResource("/xml/sql.xml")
					.getPath();
			prop = new Properties();
	...
	@Override
	public List<Todo> todoListFullView(Connection conn) throws Exception{// Controller까지 던지는 Exception
		// 결과 저장용 변수 선언
		List<Todo> todoList = new ArrayList<>();
		try {
			String sql = prop.getProperty("todoListFullView");
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql); //SELECT 수행후 ResultSet 반환
			/*
