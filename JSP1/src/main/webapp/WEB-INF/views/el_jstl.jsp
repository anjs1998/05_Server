...
<body>
	...
	<pre>
		...
		-JSP 에서 표현식을 간단하고 효육적으로 작성할수 있도록 고안된 언어
		(JSP 내부에 기본 내장되어 있음)
		
		-기본 작성법 \${key } (백슬래시는 제거후 사용)
	</pre>
	<h3>전달받은 파라미터를 출력하는 방법</h3>
	<p> 주소 되에 쿼리스트링(?name=홍길동&age=20)
	직접 작성하면서 테스트
	</p>
	...
	<div>
		...
		<br>
		age : <%=request.getParameter("age") %> <!-- null 표시 뜸 -->
	</div>
	<h4> 2. EL </h4>
	...
	<div>
		...
		name : ${param.name}
		<br>
		age : ${param.age} <!-- null 표시 안뜸 -->
	</div>
	
	<hr><hr>
	<h1> JSTL (Jsp Standard Tag Library)</h1>
	<pre> JSP에서 자주 사용하는 Java 코드를 
	(if, for, 변수 선언..)
	스크립틀릿이 아닌 HTML 태그 모양으로 작성할수 있도록
	태그를 제공하는 라이브러리
	...
	<c:if test = "${param.age > 20 }"> <!-- 쌍따옴표와 괄호 사이 공백 있으면 안됨! -->
		<h3> 성인입니다.(JSTL 사용)</h3>
	</c:if>
